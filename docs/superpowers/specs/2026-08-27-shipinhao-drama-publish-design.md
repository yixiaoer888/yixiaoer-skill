...
`--keyword` 作为现有查询命令的兼容别名，内部统一使用 `keyWord` 查询参数。命令调用 `Client.DramaTasks(accountID, keyword)`，访问：

```text
/platform-accounts/{accountID}/drama-tasks?keyWord={keyword}
```

查询结果只做现有 `queryData` 的 envelope 解包，不改变接口返回的剧集对象，不补造 `raw`，也不把剧集对象压缩成只含 ID 的对象。这样 `--value-file` 可以直接复用 `yxer query drama-tasks ... --json` 的结果，`publish form choose` 仍可通过 `yixiaoerId` 选择候选。
...
`drama` 不加入现有要求 `raw` 的通用动态对象字段列表，也不加入合集归一化逻辑。预校验仍会拒绝未声明字段和占位符；`yixiaoerImageUrl` 按平台元数据地址处理，沿用现有 `shouldIgnoreExternalURLPath` 规则，不会被当成待上传的视频或封面外链。
...
2. `query` 命令暴露 `drama-tasks`，支持 `--query` 和 `--keyword`，stdout 仍只输出 JSON。
3. 视频号 `schema fields` 返回 `drama` 字段、正确路径、三个字段定义和 `drama-tasks` 查询命令；动态示例不包含 `raw`。
4. 视频号 schema 接受来自查询结果的完整 `drama` 对象，拒绝缺少三个字段或包含未声明子字段的对象。
5. `publish form choose` 能从 `drama-tasks` 查询 envelope 中按 `yixiaoerId` 选择候选，并记录 `sourceCommand`、目标账号和字段路径。
6. preflight 不因 `drama.yixiaoerImageUrl` 是 HTTP 地址而拒绝 payload，也不要求 `drama.raw`。
7. dry-run 和真实发布 body 都保留 `contentPublishForm.drama` 的三个字段，不转换成 `collection` 或丢弃图片地址。
8. 多候选剧集未指定 ID/index 时会阻止选择；不存在的 ID 返回结构化错误。
9. 既有 `collection` 查询、合集 payload 和其他平台测试保持通过。
10. 全量 Go 测试、CLI schema/query 命令测试通过，且工作区 stdout/stderr 输出契约不回归。
