...
5. **分类下钻准则 (Global Requirement)**：若图文发布涉及分类（Category），Agent **必须** 引导至最深层的叶子节点（二级分类）。严禁仅停留在父级大类。
...
| **发布平台不在支持列表** | 账号权限不匹配（如某些仅支持视频的账号被用于发布图文）。 | **核实能力**：通过 `query-accounts` 确认账号支持的任务类型是否包含“图文”。 |
