...
### 8. 发布平台不在支持列表 (Platform Type Mismatch)
**现象描述：** 在执行发布任务时，接口返回错误：“发布平台不在支持列表”。

> [!WARNING]
> **根本原因：**
> *   **发布能力冲突：** 所选账号在蚁小二中支持的任务类型（如“图文”、“视频”）与当前执行的发布动作（如“文章发布”）不匹配。
> *   **典型案例：** 小红书账号通常仅支持“图文”或“视频”发布。若 Agent 尝试通过“文章发布”流程 (`publishType: "article"`) 调用小红书账号，系统会因类型不兼容而报错。
>
> **解决方案：**
> *   **账号能力对标：** 在执行发布前应当通过 `query-accounts` 接口核实账号的发布权限，确保动作与账号能力匹配。
> *   **流程修正：** 若发生不匹配（如想发内容到小红书），应将内容适配为该账号支持的类型（如转换为“图文”形式）后再次发布。
> *   **智能过滤：** Agent 在执行多平台混发时，应自动筛选出支持该内容类型的账号，并对不支持的账号进行跳过或提示。
