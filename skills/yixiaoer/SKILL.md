...
- 淘宝光合 `shopping_cart` 必须先通过目标账号、相同发布类型的 `taobao-guanghe-goods-tabs` 查询可用来源，再使用显式 `--source` 执行 `yxer query taobao-guanghe-goods`；完整保留商品对象和 `raw`，每个账号最多 6 件；不得依赖省略来源时的服务端默认商品池。
