...
const SAFE_PATH_SEGMENT_RE = /^[A-Za-z0-9_-]{1,128}$/;
const SAFE_BUCKET_RE = /^[A-Za-z0-9_-]{1,64}$/;
const ALLOWED_UPLOAD_PROTOCOLS = new Set(['https:']);
const DEFAULT_UPLOAD_ROOT = process.cwd();

const QUERY_ALLOWLISTS: Record<string, string[]> = {
  accounts: ['platform', 'platforms', 'platformType', 'name', 'group', 'groupId', 'page', 'size', 'status', 'loginStatus', 'isolation', 'parentId', 'time', 'keyword', 'keyWord'],
  records: ['page', 'size', 'status', 'platform', 'platforms', 'publishType', 'startTime', 'endTime', 'start_time', 'end_time', 'keyword', 'keyWord'],
  'content-overviews': ['page', 'size', 'platform', 'platformAccountId', 'publishUserId', 'type', 'title', 'publishStartTime', 'publishEndTime'],
  'account-overviews': ['page', 'size', 'platform', 'name', 'group', 'loginStatus', 'memberIds']
};

function assertSafePathSegment(value: unknown, fieldName: string): string {
  if (typeof value !== 'string' && typeof value !== 'number') {
    throw new Error(`Invalid ${fieldName}: must be a string or number`);
  }

  const text = String(value);
  if (!SAFE_PATH_SEGMENT_RE.test(text)) {
    throw new Error(`Invalid ${fieldName}: only letters, numbers, "_" and "-" are allowed`);
  }

  return encodeURIComponent(text);
}

function assertSafeBucket(bucket: unknown): string {
  if (typeof bucket !== 'string' || !SAFE_BUCKET_RE.test(bucket)) {
    throw new Error('Invalid bucket: only letters, numbers, "_" and "-" are allowed');
  }
  return bucket;
}
...
function resolveUploadPath(urlOrPath: string): string {
  const uploadRoot = path.resolve(process.env.YIXIAOER_UPLOAD_ROOT || DEFAULT_UPLOAD_ROOT);
  const absolutePath = path.resolve(uploadRoot, urlOrPath);

  if (!isPathInside(absolutePath, uploadRoot)) {
    throw new Error(`Invalid local file path: path must stay within upload root ${uploadRoot}`);
  }
  ...
  if (!stat.isFile()) {
    throw new Error(`Invalid local file path: expected a file`);
  }
...
async function assertSafeRemoteUrl(rawUrl: string): Promise<URL> {
  ...
  if (!ALLOWED_UPLOAD_PROTOCOLS.has(urlObj.protocol)) {
    throw new Error('Invalid upload url: only https URLs are allowed');
  }

  if (urlObj.username || urlObj.password) {
    throw new Error('Invalid upload url: credentials are not allowed');
  }

  const records = await dns.promises.lookup(urlObj.hostname, { all: true, verbatim: true });
  if (records.length === 0 || records.some(record => isPrivateIp(record.address))) {
    throw new Error('Invalid upload url: private or local network addresses are not allowed');
  }

  return urlObj;
}

function appendAllowedQueryParams(targetUrl: URL, payload: Record<string, any>, action: string) {
  const allowed = new Set(QUERY_ALLOWLISTS[action] || []);

  Object.keys(payload).forEach(key => {
    if (key === 'action') return;
    if (!allowed.has(key)) {
      throw new Error(`Unsupported query parameter for ${action}: ${key}`);
    }

    const value = payload[key];
    if (Array.isArray(value)) {
      value.forEach((item: any) => targetUrl.searchParams.append(key, String(item)));
...
export async function uploadResource(
  ...
  if (typeof urlOrPath !== 'string' || !urlOrPath) {
    throw new Error('Missing required url for upload');
  }

  if (/^https?:\/\//i.test(urlOrPath)) {
    const urlObj = await assertSafeRemoteUrl(urlOrPath);
    const res = await fetch(urlObj.toString(), { redirect: 'error' });
    ...
    const absolutePath = resolveUploadPath(urlOrPath);
  ...
  const uploadInfo = await callApi(`/storages/${safeBucket}/upload-url?${queryParams.toString()}`);
  ...
  try {
    ...
    switch (action) {
      ...
      case 'accounts': // 账号列表
        ...
        appendAllowedQueryParams(accountUrl, payload, action);
      ...
      case 'records': // 发布记录
        ...
        appendAllowedQueryParams(recordUrl, payload, action);
      ...
      case 'details': // 任务详情
        ...
        result = await callApi(`/v2/taskSets/${assertSafePathSegment(payload.task_set_id, 'task_set_id')}/tasks`, { method: 'GET' });
        break;

      case 'categories': // 分类查询
        const categoryUrl = new URL(`${API_URL}/platform-accounts/${assertSafePathSegment(payload.account_id, 'account_id')}/categories`);
      ...
      case 'activities': // 活动查询
        const activityUrl = new URL(`${API_URL}/platform-accounts/${assertSafePathSegment(payload.account_id, 'account_id')}/activities`);
      ...
      case 'locations': // POI 搜索
        const locationUrl = new URL(`${API_URL}/platform-accounts/${assertSafePathSegment(payload.account_id, 'account_id')}/location`);
      ...
      case 'music': // 音乐素材
        ...
        const musicUrl = new URL(`${API_URL}/platform-accounts/${assertSafePathSegment(payload.account_id, 'account_id')}/music`);
      ...
      case 'music-category': // 音乐分类
        ...
        result = await callApi(`/platform-accounts/${assertSafePathSegment(payload.account_id, 'account_id')}/music/category`, { method: 'GET' });
        break;

      case 'collections': // 合集查询
        const collectionUrl = new URL(`${API_URL}/platform-accounts/${assertSafePathSegment(payload.account_id, 'account_id')}/collections`);
      ...
      case 'update-account': // 更新账号信息 (如设置代理)
        ...
        result = await callApi(`/platform-accounts/${assertSafePathSegment(payload.account_id, 'account_id')}`, {
          method: 'PATCH',
      ...
      case 'content-overviews': // 作品数据
        ...
        appendAllowedQueryParams(contentOverviewUrl, payload, action);
      ...
      case 'account-overviews': // 账号数据 (V2)
        ...
        appendAllowedQueryParams(accountOverviewUrl, payload, action);
        result = await callApi(accountOverviewUrl.toString(), { method: 'GET' });
        break;

      case 'groups': // 群聊列表
        result = await callApi(`/platform-accounts/${assertSafePathSegment(payload.account_id, 'account_id')}/group-chats`, { method: 'GET' });
        break;

      case 'goods': // 商品列表
        const goodsUrl = new URL(`${API_URL}/platform-accounts/${assertSafePathSegment(payload.account_id, 'account_id')}/goods`);
      ...
      case 'hot-events': // 热点列表
        const hotEventUrl = new URL(`${API_URL}/platform-accounts/${assertSafePathSegment(payload.account_id, 'account_id')}/hot-events`);
      ...
      case 'challenges': // 挑战列表
        const challengeUrl = new URL(`${API_URL}/platform-accounts/${assertSafePathSegment(payload.account_id, 'account_id')}/challenges`);
      ...
      case 'miniapps': // 小程序列表
        const miniappUrl = new URL(`${API_URL}/platform-accounts/${assertSafePathSegment(payload.account_id, 'account_id')}/mini-apps`);
      ...
      case 'syncapps': // 同步应用列表
        result = await callApi(`/platform-accounts/${assertSafePathSegment(payload.account_id, 'account_id')}/sync-apps`, { method: 'GET' });
        break;

      case 'games': // 游戏列表
        const gameUrl = new URL(`${API_URL}/platform-accounts/${assertSafePathSegment(payload.account_id, 'account_id')}/games`);
