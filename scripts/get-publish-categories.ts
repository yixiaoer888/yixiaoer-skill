 ...
 * 调用方式: node scripts/get-publish-categories.ts --payload='{"account_id":"XXX","type":"article"}'
 */
...
async function main() {
  ...
  try {
    ...
    const response = await fetch(`${API_URL}/web/config-data/category-tasks`, {
      method: 'POST',
      headers: {
      ...
      body: JSON.stringify({
        ...
        publishType: payload.type || 'video'
      })
    });
