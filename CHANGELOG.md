...
- 修复大鱼号视频发布失败：按实际契约移除必填描述，调整标题、标签和分类约束，并将蚁小二 Web 固定分类目录嵌入 CLI，查询和准备流程不再调用不支持大鱼号的账号分类接口。
- 修复视频号视频无法挂商品：`shopping_cart` 改为接收 `yxer query goods` 返回的单个 GoodListItemResponse，不再错误要求数组或 `raw`；同时兼容旧的 `shoppingCart` 字段并在发布前归一化到标准字段。
...
- 淘宝光合页面式表单来源校验要求商品查询命令包含 `--source`，并为缺失来源返回稳定的结构化错误和下一步命令。
- 更新 Schema、Skill 与命令文档，要求先查询 `goods-tabs`，再从当前账号和发布类型的真实来源中选择商品；不全局禁止 `coreitem`。

## [3.2.16] - 2026-09-14

- 新增淘宝光合视频、图文发布，专属商品分类/商品查询、挂车来源校验、严格 preflight、Schema 与平台文档。
- 淘宝光合视频和图文发布支持本机通道（`--publish-channel local`）。
