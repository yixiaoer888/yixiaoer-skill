...
- **文章发布封面字段校验修复**
  - 百家号、头条号文章 schema 的封面字段从单个 `cover` 修正为 `covers` 数组，贴合实际发布 payload
  - 百家号文章支持 1-3 张封面，头条号文章支持 1-9 张封面
  - 更新验证测试，确保标准发布请求与百家号分类路径数组场景均接受 `covers`

- **查询别名与平台映射行为修复**
  - 统一 `--query` / `--keyword` 别名解析，避免兼容入口读取错误的 flag 存储
  - 修复平台名规范化相关问题，降低中文平台名、别名进入远程查询时的漂移风险

### 🎉 新增功能

- **旧版查询能力迁移进 CLI**
  - `yxer query` 新增并统一承载多类动态对象查询：小程序、同步应用、游戏、热点、群聊、活动、音乐分类、发布详情、账号概览、作品概览、代理列表、代理地区等
  - 位置、音乐、商品、挑战等查询补充分页参数 `--next-page`，音乐查询补充 `--category-id` / `--category-name`
  - 新增账号更新能力 `update-account`，支持代理、快代理地区、备注、分组更新，并提供 `--dry-run` 输出预览

### 🔧 技术改进

- **工作流结果信封收敛**
  - 发布 `--dry-run` 通过统一 envelope 返回 `publish.dry-run`，并显式包含 `dryRun: true`、`payload`、`meta`
  - 查询、账号、草稿、素材、上传、schema、validate 等命令继续使用结构化 JSON 输出，减少 Agent 解析分支

- **运行时依赖注入与命令瘦身**
  - 引入 `internal/app.Runtime`，集中注入配置、API client、输出等共享依赖
  - 发布、查询、doctor 等命令下沉到 workflow/service 层，逐步弱化旧 `internal/core/*` facade

### 📚 文档

- 统一 `yxer query ...` 入口文档，保留旧命令作为兼容入口并在帮助文案中标注推荐路径
- 更新动态对象查询参考文档，覆盖挑战、合集、商品、位置、音乐、分类、话题等发布前置数据获取方式
- 补充文章封面字段文档，明确百家号等文章发布使用 `covers`
