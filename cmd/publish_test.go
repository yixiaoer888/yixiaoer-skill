...
func TestPublishCommandKeepsJianshuArticleContentInForm(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "/v2/platform/accounts":
			_ = json.NewEncoder(w).Encode(map[string]interface{}{
				"data": []map[string]interface{}{{"platformAccountId": "acc_jianshu_1", "name": "简书账号", "status": 1}},
			})
		case "/taskSets/v2":
			publishCalls++
			if err := json.NewDecoder(r.Body).Decode(&publishBody); err != nil {
			...
			_ = json.NewEncoder(w).Encode(map[string]interface{}{"data": map[string]interface{}{"taskSetId": "task_set_jianshu_1"}})
		default:
			t.Fatalf("unexpected path: %s", r.URL.Path)
		}
	}))
